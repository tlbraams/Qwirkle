...
public class AnsiClient extends Thread {
	...
	public static void main(String[] args) {
		...
		try {
			AnsiClient client = new AnsiClient();
			client.start();
		} catch (IOException e) {
	 ...
	 */
	public AnsiClient() throws IOException {
	...
	public void displayPlayerMenu(/*@ non_null */String nameOfClient,
		...
		while (running) {
			...
			if (kindOfPlayer.equals("1")) {
				player = new AnsiHumanPlayer(nameOfClient, playerNumber);
				running = false;
	...
	public void startGame(String line) {
		...
		view = new AnsiTUI(board, players.size());
